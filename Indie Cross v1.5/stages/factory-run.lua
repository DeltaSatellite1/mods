...
function onCreate()
	createBG(curState)
	local willHaveTransition = false
	--Precache
	if willHaveTransition then
		...
		if not lowQuality then
			...
			precacheImage('bendy/stairs/gradient')
			precacheImage('bendy/gay/C_06_BLEND_MODE_ADD')
		end
	end
end
function createBG(state)
	...
	elseif state == 3 then
		...
		for ground = 1,2 do
			makeLuaSprite('GayBGGround'..ground, 'bendy/gay/C_01', -1500,20)
...
function onUpdate(el)
	if curState == 0 then
		if (getProperty('BendyBGRun.animation.curAnim.finished')) then
			...
			playAnim('BendyBGRun', 'Loop' ..animCounter, false)
		end
	elseif curState == 2 or curState == 3 then
		if curState == 2 then
			local starsSpeed = 1500*el
			...
			setProperty('stairsChainleft1.y',chainY + starsSpeed + (1000*el))
			setProperty('stairsChainright1.y',chainRight + starsSpeed + (1000*el))
			for chain = 2,3 do
